...
| `-short` | exactly 1 image (`.png`/`.jpg`/`.jpeg`) + exactly 1 audio-only file supported by ffmpeg | one `_8K_Short.mp4` portrait clip (H264 ladder, 4320x7680), capped at 58 s, image fitted with black padding |
| `-nfttoshort` | any single audio-only file supported by ffmpeg + `Vertical_8K.png` or existing/derived `*_NFT8K.png` (or any single image as fallback) | one `_Short.mp4` portrait clip, capped at 58 s, source loudness preserved |
...
|---|---|---|
| `-full` / `-run` | exactly 1 source audio (`.flac`/`.wav`/`.mp3`) + 1 source image or direct 8K PNGs in `SRC_DIR` | image deliverables, WAV/M4A/MP3, `*_RF64`/`*_BW64` `.wav`/`.flac` archival companions, main MP4, short MP4 |
